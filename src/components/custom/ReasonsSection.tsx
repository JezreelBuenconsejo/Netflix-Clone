      ...
      <h2 className="text-2xl  font-medium text-white mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {reasons.map((reason, index) => (
            <Card
              key={index}
              className="border-none shadow-lg hover:shadow-xl transition-shadow duration-300 py-0"
              style={{
                background:
                  "linear-gradient(149deg, #192247 0%, #210e17 96.86%)",
              }}
            >
              <CardContent className="px-4 py-6 flex flex-col h-full">
                <div className="flex flex-col gap-20 items-center h-full relative">
                  <div>
                    <h3 className="text-2xl font-semibold text-white mb-3 mr-10">
                      {reason.title}
                    </h3>

                    <p className="text-white/70 text-base mr-5">
                      {reason.description}
                    </p>
                  </div>
                  <div className="mt-auto ml-auto">{reason.icon}</div>
                </div>
              </CardContent>
            </Card>
          ))}
